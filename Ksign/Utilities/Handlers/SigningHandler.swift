...
final class SigningHandler: NSObject {
	...
	func modify() async throws {
        ...
        // Merge the Default Tweaks profile's always-inject files with this
        // app's per-session selection (de-duped by filename), then inject once.
        //
        // Injecting exactly once is deliberate. The previous version called
        // _inject up to twice over the same list, which appended a second
        // LC_LOAD_DYLIB for every dylib — a duplicate load command in the
        // Mach-O. TweakHandler.getInputFiles() runs _checkEllekit() at the top
        // regardless of the list, so the Substrate→ElleKit path is still
        // covered by a single call.
        let injectionFiles = _resolvedInjectionFiles(for: _options)
        try await _inject(for: movedAppPath, with: injectionFiles, with: _options)
...
extension SigningHandler {
	...
	/// Combines this app's per-session injection files with the tweaks enabled
	/// in the Default Tweaks profile.
	///
	/// The default profile stores bare filenames and they're resolved against
	/// the Tweaks directory here, at sign time — matching by filename rather
	/// than absolute URL so the selection survives an app reinstall (which
	/// changes the container UUID, and therefore every absolute path). Anything
	/// already present in the per-session list, or missing from disk, is
	/// skipped so nothing gets injected twice.
	private func _resolvedInjectionFiles(for options: Options) -> [URL] {
		var files = options.injectionFiles
		var seenNames = Set(files.map { $0.lastPathComponent })

		let tweaksDir = _fileManager.tweaks
		for name in (options.defaultInjectionDylibs ?? []) {
