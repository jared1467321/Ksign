...
struct DownloaderView: View {
    ...
    // MARK: Edit Mode
    // Selection is keyed by localPath rather than DownloadItem.id, because
    // ids are regenerated every time loadDownloadedIPAs() reloads the list —
    // paths stay stable across reloads.
    @State private var _isEditMode: EditMode = .inactive
    @State private var _selectedItems: Set<URL> = []
            ...
            .toolbar {
                ...
                if _isEditMode.isEditing {
                    ToolbarItemGroup(placement: .topBarTrailing) {
                        Button {
                            _bulkImportSelectedToLibrary()
                        } label: {
                            NBButton(.localized("Import to Library"), systemImage: "square.grid.2x2.fill", style: .icon)
                        }
                        .disabled(_selectedItems.isEmpty)
                        
                        Button {
                            _bulkDeleteSelectedItems()
                        } label: {
...
private extension DownloaderView {
    // Paths of the finished downloads currently visible (respects the
    // search filter, so "Select All" only grabs what the user can see).
    var _visiblePaths: [URL] {
        filteredDownloadItems.map { $0.localPath }
    }
    
    var _isAllSelected: Bool {
        let paths = _visiblePaths
        return !paths.isEmpty && paths.allSatisfy { _selectedItems.contains($0) }
    }
    ...
    func _toggleSelectAll() {
        ...
        withAnimation(.spring(response: 0.4, dampingFraction: 0.8, blendDuration: 0)) {
            if _isAllSelected {
                paths.forEach { _selectedItems.remove($0) }
            } else {
                _selectedItems.formUnion(paths)
            }
        }
    }
    
    func _resolveSelectedItems() -> [DownloadItem] {
        downloadManager.finishedItems.filter { _selectedItems.contains($0.localPath) }
    }
    
    func _bulkImportSelectedToLibrary() {
        for item in _resolveSelectedItems() {
