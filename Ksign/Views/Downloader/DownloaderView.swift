...
struct DownloaderView: View {
    ...
    // Hard input block: every bar button is `.disabled` while this is true, which
    // switches off the underlying UIBarButtonItem so no touch is delivered at
    // all. Cleared one second later. See LibraryView for the full note.
            ...
            .toolbar {
                ToolbarItemGroup(placement: .topBarLeading) {
                    if !downloadManager.finishedItems.isEmpty || _isEditMode.isEditing {
                        // Blocked for a second too — the edit-mode transition is
                        // animated the same way the selection change is.
                        EditButton()
...
private extension DownloaderView {
    ...
    // Shuts the whole toolbar for one second.
    func _blockBarInput() {
    ...
    var _selectAllButton: some View {
        Button {
            ...
            guard !filteredDownloadItems.isEmpty, _tapGate.allow() else { return }
            // Block first, then work — the runloop deferral that used to be here
            // has been dropped. See LibraryView.
            _blockBarInput()
