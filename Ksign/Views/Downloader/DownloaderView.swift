...
private extension DownloaderView {
        ...
        Task {
            await withTaskGroup(of: Void.self) { group in
                ...
                while next < initial {
                    ...
                    group.addTask { await Self._importOneDownload(item) }
                }
    ...
    static func _importOneDownload(_ file: DownloadItem) async {
        let manager = DownloadManager.shared
        let id = "FeatherManualDownload_\(UUID().uuidString)"
        let dl = await MainActor.run { manager.startArchive(from: file.url, id: id) }
