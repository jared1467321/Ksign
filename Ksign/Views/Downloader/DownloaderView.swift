                ...
                ToolbarItemGroup(placement: .topBarLeading) {
                    ...
                    if _isEditMode.isEditing {
                        Button {
                            ...
                            Text(_allDownloadsSelected ? .localized("Deselect All") : .localized("Select All"))
                        }
                        .disabled(filteredDownloadItems.isEmpty)
                    }
                }
                ...
                if _isEditMode.isEditing {
                    ToolbarItem(placement: .topBarTrailing) {
                        Button {
                            ...
                            NBButton(.localized("Import"), systemImage: "tray.and.arrow.down.fill", style: .icon)
                        }
                        .disabled(_selectedDownloads.isEmpty)
                    }
                    ToolbarItem(placement: .topBarTrailing) {
                        Button {
                            _bulkDeleteSelected()
...
private extension DownloaderView {
    ...
    func _toggleSelectAllDownloads() {
        ...
        withAnimation {
            if _allDownloadsSelected {
                ids.forEach { _selectedDownloads.remove($0) }
            } else {
                ids.forEach { _selectedDownloads.insert($0) }
            }
        }
    }
    ...
    func _bulkImportSelected() {
        ...
        Task {
            ...
            let token = await MainActor.run { DownloadManager.shared.beginImportBatch(count: items.count) }
            ...
            // Normally a no-op — every item has checked in by now. It matters
            // if the batch was cut short.
            await MainActor.run { DownloadManager.shared.endImportBatch(token) }
        }
    }
