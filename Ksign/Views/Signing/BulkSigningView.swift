...
struct BulkSigningView: View {
	...
	// When true, the just-signed apps are handed off for installation
	// once the whole queue finishes.
	var signAndInstall: Bool = false

	init(apps: [AppInfoPresentable], signAndInstall: Bool = false) {
		self.apps = apps
		self.signAndInstall = signAndInstall
		let storedCert = UserDefaults.standard.integer(forKey: "feather.selectedCert")
		__temporaryCertificate = State(initialValue: storedCert)
		
		let defaultOptions = OptionsManager.shared.options.mergingDefaultTweaks()
...
extension BulkSigningView {
	...
	private func _start() {
		...
		// Capture these once up front. The certificate selection can't
		// change while signing is in progress, and copying the configs
		// keeps the loop independent of any later view-state mutation.
			...
			await MainActor.run {
				if !failures.isEmpty {
					let message = failures
						...
						.joined(separator: "\n")
					UIAlertController.showAlertWithOk(title: .localized("Error"), message: message)
				}
				...
				if signAndInstall, successCount > 0 {
					...
					DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
						NotificationCenter.default.post(
							name: NSNotification.Name("ksign.bulkSignAndInstall"),
							object: count
						)
					}
				}
