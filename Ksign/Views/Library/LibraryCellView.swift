...
struct LibraryCellView: View {
	...
	/// every visible cell depend on the entire set — selecting one app
	/// invalidated all of them, and in a `List` that becomes an N-row
	/// UICollectionView batch update for a single tap.
	var isSelected: Bool
	/// Selection is owned by LibraryView; the cell just reports the tap.
        ...
        HStack(spacing: 9) {
			...
			if !isEditing {
				if app.isSigned, let certInfo = certInfo {
					...
					.padding(.vertical, 5)
					.nbThemeBackground(certInfo.role)
	...
	private func _toggleSelection() {
		guard app.uuid != nil else { return }
		
		let impactFeedback = UIImpactFeedbackGenerator(style: .light)
		impactFeedback.impactOccurred()
		
		// Deliberately NOT wrapped in withAnimation. A spring around a
		// selection change pushes the List onto UIKit's animated
		// performBatchUpdates path — the exact stack the watchdog has been
		// killing. The checkmark swap reads fine without it.
		onToggleSelection()
	}
}
