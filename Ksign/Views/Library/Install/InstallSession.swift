...
final class InstallSession: ObservableObject {
	...
	func start(apps: [AppInfoPresentable]) {
		...
		if jobs.isEmpty {
			...
			_batchGroupSize = stored == 0 ? 5 : min(10, max(1, stored))

			// Concurrency must match the group size: a job can't reach `.ready` to
			// join a group without a slot, so a limit below the group size would
			// keep a full group from ever assembling.
			InstallQueueCoordinator.shared.setMaxConcurrent(_willBatch ? _batchGroupSize : 3)
		}
