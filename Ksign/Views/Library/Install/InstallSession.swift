...
final class InstallSession: ObservableObject {
	...
	private func _startTicking() {
		...
		_tickTask = Task { @MainActor [weak self] in
			while !Task.isCancelled {
				...
				// The keep-alive's Dynamic Island pill is indeterminate by
				// default, because most of what claims it — signing, import,
				// extraction — has no single number to report. A batch install
				// does, and it's already computed just above, so hand it over
				// and let the ring mean something.
				if #available(iOS 26.0, *) {
					KeepAliveTaskManager.shared.report(.bulkInstalls, progress: self?.aggregateProgress)
				}
	...
	private func _stopTicking() {
		...
		// Withdraw the figure as well as stopping the tick. Leaving the last
		// value behind would pin the Dynamic Island ring at wherever the batch
		// happened to end while some other owner carried on holding the
		// keep-alive.
		if #available(iOS 26.0, *) {
			KeepAliveTaskManager.shared.report(.bulkInstalls, progress: nil)
		}
	}
