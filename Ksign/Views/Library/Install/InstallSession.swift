...
final class InstallSession: ObservableObject {
	...
	func start(apps: [AppInfoPresentable]) {
		...
		//
		// The flag keeps this balanced: `start(apps:)` also runs when apps are
		// added to a batch that's already going, and the manager counts its
		// holders now — claiming twice and releasing once would leave the
