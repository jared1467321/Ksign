...
struct InstallProgressView: View {
	...
	// the unambiguous "this one is going" marker: a pumpkin dot wrapped in a
	// glow that pulses, so an in-progress app is easy to pick out at a glance.
	//
	// Pumpkin rather than green on purpose. Green now means "done" everywhere
	// else in the app, and it's also the tint, so a green dot on a green-tinted
	// grid is the one thing here that shouldn't blend in. Orange is the only
	// colour on this screen that isn't the accent.
	private var _activityDot: some View {
		let dot = NBHalloween.warning
		return ZStack {
