...
struct InstallProgressView: View {
	...
	// the unambiguous "this one is going" marker: a green dot wrapped in a glow
	// that pulses, so an in-progress app is easy to pick out at a glance.
	private var _activityDot: some View {
		let green = Color(red: 0.26, green: 0.94, blue: 0.42)
		return ZStack {
			...
			Circle()
				...
				.frame(width: 13, height: 13)
				.shadow(color: green.opacity(0.9), radius: 4)
		}
