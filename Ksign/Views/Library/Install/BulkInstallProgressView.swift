...
struct BulkInstallProgressView: View {
    ...
    var body: some View {
        ...
        .onReceive(viewModel.$status) { newStatus in
            ...
            // Server method only. The idevice path already gets real progress
            // from installation_proxy's callback, so running this too means two
            // writers fighting over `installProgress` — and this loop's own
            // completion guess can end the row early.
    ...
    private func _install() {
        Task.detached {
            do {
                ...
                // This used to restart the heartbeat and say nothing, so a
                // failed install was indistinguishable from one still running.
                // `.broken` also lets the status handler free the queue slot.
    ...
    private func _startStallWatchdog() {
        ...
        _watchdogTask = Task { @MainActor in
            while !Task.isCancelled {
                ...
                // `uploadProgress`, the server path reports via `installProgress`.
                let progress = max(viewModel.installProgress, viewModel.uploadProgress)
                ...
                if _installationMethod == 1 {
                    ...
                    // So report it, which frees the slot for the rest of the batch.
                    Logger.misc.info("idevice install for \(app.identifier ?? "?") stalled ~\(Int(stalledFor))s — reporting")
                    viewModel.status = .broken(InstallQueueError.stalled)
                    break
                }

                // Server method: only re-fire when the prompt itself never
                // landed. Re-firing mid-transfer or mid-install is what produced
                // duplicate prompts on apps that were working fine — there is no
                // progress signal during `.sendingPayload`, so a large IPA looks
                ...
                if _retryCount < _maxRetries {
                    _retryCount += 1
                    _lastActivity = Date()
                    Logger.misc.info("Install for \(app.identifier ?? "?") prompt never landed after ~\(Int(stalledFor))s — refiring (attempt \(_retryCount)/\(_maxRetries))")
