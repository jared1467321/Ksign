...
final class InstallJob: ObservableObject, Identifiable {
	...
	func start() {
		...
		_installTask = Task { @MainActor [weak self] in
			...
			guard let self else {
				// Claimed a slot for a job that's gone — hand it straight back
				// or the queue leaks a slot and everything behind it stalls.
	...
	private func _install() {
		...
		Task.detached { [weak self] in
			do {
				...
				if method == 0 {
					await MainActor.run {
						self?.installer.packageUrl = packageUrl
