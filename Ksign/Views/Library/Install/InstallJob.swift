...
final class InstallJob: ObservableObject, Identifiable {
	...
	func retry() {
		...
		// The dividing line is whether the expensive work is already done, not
		// whether a slot is currently held — those came apart once batched jobs
		// started releasing their slot the moment they parked at the prompt. A
		// job with a built package (the dismissed-prompt case, batched or not)
		// has its server up and needs nothing from the queue, so it re-fires the
		// prompt instantly whether or not it still holds a slot. A job that's
		// mid-build still holds its slot and simply carries on. Only a job that
		// never got a package — and isn't holding a slot to keep building with —
		// has to queue up again like anyone else.
		//
	...
	private func _handleStatus(_ newStatus: InstallerStatusViewModel.InstallerStatus) {
		if case .ready = newStatus {
			...
			case .pending:
				...
				// one shared prompt.
				batchRole = .awaiting
				// The archive is done and the server is up, so this job no longer
				// needs a build slot — hand it back now rather than clutching it
				// until the whole group installs. This is what lets a group be
				// larger than the slot count: parked apps free the slots that the
				// still-queued members need in order to build and join. The
				// server keeps serving regardless; slots gate building, not the
				// prompt or the install that follows.
				_releaseSlotIfNeeded()
