...
final class InstallJob: ObservableObject, Identifiable {
	...
	// Generous, so iOS working sequentially through several payloads isn't
	// mistaken for a skip.
	private let _batchMemberFallback: TimeInterval = 45

	// A shared (host) prompt covers several apps, so it takes longer to review
	// and tap than a single-app one — and iOS takes longer to pull several
	// payloads. Wait well past the single-app window before assuming the prompt
	// was dropped and re-showing it.
	private let _batchHostStallTimeout: TimeInterval = 90
	...
	private func _startStallWatchdog() {
		...
		_watchdogTask = Task { @MainActor [weak self] in
			while !Task.isCancelled {
				...
				if self._fastFailWindow {
					timeout = 15.0
				} else if self.batchRole == .host {
					// Shared prompt: give the user (and iOS) longer before re-showing.
					timeout = self._batchHostStallTimeout
				} else {
