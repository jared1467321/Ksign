...
final class InstallJob: ObservableObject, Identifiable {
	...
	// MARK: Install queue
	private var _hasSlot = false
	private var _slotReleased = false
	...
	func start() {
		...
		_installTask = Task { @MainActor [weak self] in
			...
			// the claim and this check. Returning without releasing leaves the
			// queue permanently one slot smaller.
	...
	private func _startStallWatchdog() {
		...
		_watchdogTask = Task { @MainActor [weak self] in
			while !Task.isCancelled {
				...
				guard stalledFor >= self._stallTimeout else { continue }
...
final class InstallQueueCoordinator {
	...
	// false if the calling task is cancelled while waiting — in that case no
	// slot is claimed, so nothing leaks. Because this runs on the main actor
	// the check-and-claim is atomic and two installs can't grab the same slot.
	func acquire() async -> Bool {
		while true {
			if Task.isCancelled { return false }
			if !_isPaused, _activeCount < _maxConcurrent {
				_activeCount += 1
				return true
			}
