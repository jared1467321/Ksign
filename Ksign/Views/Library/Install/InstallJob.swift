...
final class InstallJob: ObservableObject, Identifiable {
	...
	// True when this app is parked at a shared prompt that hasn't been accepted
	// — either dismissed, or the group was fired and iOS never pulled this item.
	...
	func rejoinPool() {
		// Shed any role from the manifest it was just in.
		if batchRole == .host { installer?.extraManifestItems = [] }
		_batchHost = nil
		_progressTask?.cancel()
		_progressTask = nil
		...
		if hasBuiltPackage {
			// Server and package are still alive — no rebuild, no slot needed.
			// Park straight back into the pool and let the session regroup it.
			batchRole = .awaiting
			phase = .running
