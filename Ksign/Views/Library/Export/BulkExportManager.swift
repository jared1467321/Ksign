...
final class BulkExportManager: ObservableObject {
	...
	// Mirrors the batch figure into the keep-alive's Dynamic Island pill, so a
	// bulk export that's been backgrounded reads as progress rather than as an
	// indeterminate spinner. Counted per app, same as the on-screen overlay —
	// the zip step for a single app has no sub-progress to offer.
	private func _reportProgress() {
		guard #available(iOS 26.0, *) else { return }
		KeepAliveTaskManager.shared.report(.bulkExport, progress: isExporting ? overallProgress : nil)
	}
	...
	func finishPicking() {
		...
		// Withdraw before the claim goes, so nothing is left pinning the ring
		// at 100% if another owner is still holding the keep-alive.
		if #available(iOS 26.0, *) {
			KeepAliveTaskManager.shared.report(.bulkExport, progress: nil)
		}
	}
