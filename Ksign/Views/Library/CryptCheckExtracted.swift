//
//  CryptCheckExtracted.swift
//  Ksign
//
//  Extracted-app variant of Crypt Check for Library .app bundles.
//
...
enum CryptCheckExtractedError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidAppBundle:
            return "The selected extracted app bundle could not be opened."
        case .noMachOBinaries:
            return "No Mach-O binaries were found in this extracted app bundle."
        }
    }
}

enum CryptCheckExtractedAnalyzer {
    ...
    // large binaries are memory-mapped directly from the extracted app bundle,
    // avoiding jetsam without penalizing devices that can scan resident bytes.
    ...
    static func generateReport(for appURL: URL) throws -> URL {
        let fileManager = FileManager.default
        var isDirectory: ObjCBool = false
        guard
            fileManager.fileExists(atPath: appURL.path, isDirectory: &isDirectory),
            isDirectory.boolValue
        else {
            throw CryptCheckExtractedError.invalidAppBundle
        }

        let hasDecryptedBy = mainInfoPlistHasDecryptedBy(in: appURL)
        ...
        let appRootPath = appURL.standardizedFileURL.path
        let appRootPrefix = appRootPath.hasSuffix("/") ? appRootPath : appRootPath + "/"
        ...
        for case let fileURL as URL in enumerator {
            do {
                ...
                guard fileSize >= 4 else { continue }

                let ext = fileURL.pathExtension.lowercased()
                if skippedExtensions.contains(ext) { continue }

                // Match the IPA scanner's behavior: probe the Mach-O magic first
                // instead of trusting extensions or executable bits.
                let prefix = try readFilePrefix(fileURL, byteCount: 4)
                guard isMachO(prefix) else { continue }

                let slices = try analyzeExtractedFile(fileURL, size: fileSize)

                let standardizedPath = fileURL.standardizedFileURL.path
                let relativePath: String
                if standardizedPath.hasPrefix(appRootPrefix) {
                    relativePath = String(standardizedPath.dropFirst(appRootPrefix.count))
                } else {
                ...
                entries.append(
                    ReportEntry(
                        name: appURL.lastPathComponent + "/" + relativePath,
                        size: fileSize,
                        slices: slices
                    )
                )
            } catch {
                // A single unreadable file should not prevent the rest of the
                // extracted app bundle from being checked.
                continue
            }
        }

        guard !entries.isEmpty else {
            throw CryptCheckExtractedError.noMachOBinaries
        }

        let html = makeHTML(
            entries: entries,
            source: appURL.lastPathComponent,
            hasDecryptedBy: hasDecryptedBy
        )
        let reportDirectory = fileManager.temporaryDirectory
            .appendingPathComponent("CryptCheckExtractedReports", isDirectory: true)
        ...
        let reportURL = reportDirectory
            .appendingPathComponent("cryptcheck_extracted_\(base)_\(stamp).html")
        try html.write(to: reportURL, atomically: true, encoding: .utf8)
        return reportURL
    }

    private static func readFilePrefix(_ fileURL: URL, byteCount: Int) throws -> Data {
    ...
    private static func analyzeExtractedFile(_ fileURL: URL, size: UInt64) throws -> [Slice] {
        if size <= memoryResidentThreshold, size <= UInt64(Int.max) {
            return analyzeMachO(try Data(contentsOf: fileURL))
        }

        // The file already exists on disk, so unlike the IPA scanner there is
        // nothing to inflate into a temporary file. Map large binaries directly.
        let mapped = try Data(contentsOf: fileURL, options: [.mappedIfSafe])
        return analyzeMachO(mapped)
    }

    private static func mainInfoPlistHasDecryptedBy(in appURL: URL) -> Bool {
        let plistURL = appURL.appendingPathComponent("Info.plist", isDirectory: false)
        guard
    ...
    private static func makeHTML(entries: [ReportEntry], source: String, hasDecryptedBy: Bool) -> String {
        ...
        <title>cryptcheck extracted report — \(escapeHTML(now))</title>
        <style>
          :root {
            color-scheme: dark;
...
struct CryptCheckExtractedReportCollection: Identifiable {
    let id = UUID()
    let reportURLs: [URL]
}

struct CryptCheckExtractedReportView: View {
    let reportURLs: [URL]
    ...
    private var navigationTitle: String {
        guard reportURLs.count > 1 else { return "Crypt Check Extracted" }
        return "Crypt Check Extracted \(selectedIndex + 1) of \(reportURLs.count)"
    }
        ...
        NavigationView {
            TabView(selection: $selectedIndex) {
                ForEach(Array(reportURLs.enumerated()), id: \.offset) { index, reportURL in
                    CryptCheckExtractedHTMLView(url: reportURL)
...
private struct CryptCheckExtractedHTMLView: UIViewRepresentable {
    let url: URL
