...
struct LibraryView: View {
	...
	@State private var _bulkSignAndInstall = false
    @State private var _isBulkInstallingPresenting = false
	@State private var _bulkInstallAfterSignRequest: BulkInstallRequest?
    ...
    var body: some View {
		NBNavigationView(.localized("Library")) {
			...
			.fullScreenCover(isPresented: $_isBulkSigningPresenting) {
				...
				}, signAndInstall: _bulkSignAndInstall)
            ...
            .sheet(item: $_bulkInstallAfterSignRequest) { request in
                BulkInstallPreviewView(apps: request.apps)
                    .presentationDetents([.medium, .large])
                    .presentationDragIndicator(.visible)
            }
			...
			.onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("ksign.bulkSignAndInstall"))) { notification in
				...
				// Item-based presentation (like the single-app install path)
				// so it reliably presents right after the signing sheet
				// dismisses — a bool-based sheet gets dropped by that race.
				_bulkInstallAfterSignRequest = BulkInstallRequest(apps: newest)
			}
        }
