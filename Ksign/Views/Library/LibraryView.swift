...
final class ToolbarTapGate {
	// Keyed per control, not one timestamp for the whole bar. A single shared
	// gate meant tapping Edit also shut the select-all circle, which is a wait
	// with no reason to exist: they are different controls driving different
	// transitions. The only thing worth refusing is the *same* control being hit
	// twice before its own transition has settled.
	private var _openAt: [String: Date] = [:]

	/// Returns true and closes that control's gate, or false if it is still shut.
	func allow(_ key: String, for interval: TimeInterval = 0.35) -> Bool {
		let now = Date()
		if let openAt = _openAt[key], now < openAt { return false }
		_openAt[key] = now.addingTimeInterval(interval)
		return true
	}
}
...
extension LibraryView {
    ...
    private var _selectAllButton: some View {
        Button {
            ...
            guard !_currentTabUUIDs.isEmpty, _tapGate.allow("selectAll") else { return }
            _toggleSelectAll()
        } label: {
