...
//
// This is the belt to the `_barInputBlocked` braces below. It refuses a second
// tap arriving inside the settle window, and it is a plain class rather than view
// state on purpose: flipping a piece of @State to track "busy" invalidates the
// toolbar, and this needs to be checkable without causing a redraw.
//
// On its own this was not enough. It stops the *action* running twice, but the
// taps still reached the button. The real block is `.disabled`, which switches
// off the underlying UIBarButtonItem so the touch is never delivered at all.
...
struct LibraryView: View {
	...
	// Hard input block for the whole toolbar. While this is true every bar button
	// is `.disabled`, which is the only thing that reliably stops the touch at the
	// UIKit level — SwiftUI maps it straight onto UIBarButtonItem.isEnabled, so
	// there is no live button to hit, never mind one mid-transition to strand.
	// Set by every bar action and by every edit-mode change, cleared one second
	// later, by which time the list has finished whatever it was doing.
	@State private var _barInputBlocked = false
		...
		NBNavigationView(.localized("Library")) {
			...
			.toolbar {
                ToolbarItemGroup(placement: .topBarLeading) {
                    // Entering and leaving edit mode is its own animated
                    // transition, so Edit is blocked for a second too — that was
                    // the original report, before Select All was in the picture.
                    EditButton()
                        .disabled(_barInputBlocked)
...
extension LibraryView {
    ...
    // UIKit state living on the bar button item, and it's one of the things that
    // gets stranded when the bar is rebuilt mid-animation — the button stays
    // greyed out and untappable until the app is relaunched, even though the
    // selection it's watching has changed. Dimming the icon and refusing the tap
    // inside the action looks identical and has no UIKit state to get stuck in.
    // Shuts the whole toolbar for one second. Nothing can restart the timer
    // early: input is already off, so no second tap can get through to try.
    private func _blockBarInput() {
        _barInputBlocked = true
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            _barInputBlocked = false
        }
    }
    ...
    private var _selectAllButton: some View {
        Button {
            ...
            // Block first, then work. The one-runloop deferral that used to be
            // here has been dropped: it pushed the list rebuild past the point
            // where the guard window opened, so the guard expired earlier
            // relative to the work finishing rather than later.
            _blockBarInput()
            _toggleSelectAll()
        } label: {
