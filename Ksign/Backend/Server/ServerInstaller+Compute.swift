	...
	var installManifest: [String: Any] {[
		"items": [[
			"assets": [
				...
				// Served from this server rather than fetched from GitHub. The
				// old URL meant every install prompt made a round trip to
				// raw.githubusercontent.com for an image already being hosted
				// on localhost — latency on every install, and a network
				// dependency on an otherwise entirely local operation.
				[
					"kind": "display-image",
