...
struct BackgroundAudioEvent: Identifiable, Equatable {
	...
	var tint: Color {
		...
		case .claimed, .reconfigured:			return .accentColor
		}
	}
}
...
final class BackgroundAudioStatus: ObservableObject {
	...
	func update(isRunning: Bool, owners: [String]) {
		DispatchQueue.main.async {
			...
			// several times a second during an install — and the manager is
			// responsible for reducing that to actual transitions.
			if #available(iOS 26.0, *) {
				KeepAliveTaskManager.shared.sync(isRunning: isRunning, owners: owners)
			}
		}
	}
