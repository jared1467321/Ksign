...
// `clonefile(2)` — declared in sys/clonefile.h, surfaced through Darwin.
import Darwin
...
extension FileManager {
	...
	//
	// `copyItem` duplicates every byte. Signing then rewrites a handful of
	// files inside that duplicate — Info.plist, the icon, embedded.mobileprovision,
	// CodeResources and the Mach-O binaries — and leaves the rest untouched,
	// so on a 400MB app roughly 650MB of that copy was written for nothing.
	//
	// `clonefile(2)` shares the underlying blocks instead. When a file is
	// modified afterwards only the changed blocks are written, and the source
	// is unaffected — which is why this is safe for signing where the hard
	// links in `ArchiveHandler` would not be. Cloning a directory clones the
	// whole hierarchy in one syscall.
	//
	// Requires both paths on the same APFS volume; everything here lives in
	// the app container, so that holds. If it doesn't, we fall back rather
	// than fail.
	func cloneItem(at srcURL: URL, to dstURL: URL) throws {
		let result = srcURL.withUnsafeFileSystemRepresentation { srcPath -> Int32 in
			guard let srcPath else { return -1 }
			
			return dstURL.withUnsafeFileSystemRepresentation { dstPath -> Int32 in
		...
		// Destination must not exist for a clone, so clear anything a partial
		// attempt left behind before falling back.
